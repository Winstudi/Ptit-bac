(() => {
  ...
  function startCountdown(payload = {}) {
    ...
    const startedAt = Number(payload.startedAt || runtimeNow());
    const durationMs = Math.max(3000, Number(payload.durationMs || 3200));
    const deadline = startedAt + durationMs;

    const update = () => {
      const remaining = deadline - runtimeNow();
      let nextValue = "3";
  ...
  window.PtitBacLobbyRuntime = Object.freeze({
    refreshModeControl: ensureRoomModeControl,
    removeCountdown
  });
})();
