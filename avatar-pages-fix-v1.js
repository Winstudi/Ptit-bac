/* =========================================================
   P'tit Bac — avatars cohérents sur tous les écrans
   ========================================================= */
(() => {
  "use strict";
  ...
  function normalizeAvatar(value, seed = "") {
    ...
    if (BASE.includes(raw)) return raw;
  ...
  function upgradeAvatarBox(box) {
    ...
    const src = normalizeAvatar(raw, seed);
  ...
  function upgradeWheelChooser() {
    ...
    const src = normalizeAvatar(chooser?.avatar, seed);
  ...
  function upgradeAll(root = document) {
    if (root instanceof Element && root.matches(AVATAR_BOXES)) {
      upgradeAvatarBox(root);
    }

    root.querySelectorAll?.(AVATAR_BOXES).forEach(upgradeAvatarBox);
    upgradeWheelChooser();
  }
