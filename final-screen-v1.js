...
function renderFinishedV2(){
 ...
 const mine=ranked.find(p=>p.id===session.playerId);
 const personalRank=mine?rank(mine):null;
 ...
 const ordinal=n=>n===1?"1 er":n+" eme";
 ...
 (mine?'<section class="fin-personal">'+avatar(mine)+'<div><small>Ta position</small><strong>'+ordinal(personalRank)+'</strong></div><b>'+points(mine)+' point'+(points(mine)>1?'s':'')+'</b></section>':'')+
