...
/* =========================================================
   E1 — validation de la migration source
   ========================================================= */

function assertE1(condition, message) {
  if (!condition) {
...
function parseJavaScript(filePath) {
  ...
  // Validation syntaxique uniquement : le code navigateur n'est pas exécuté.
  new Function(source);
  return source;
}

function validateE1Migration() {
  const appSource = parseJavaScript(appPath);
  const wheelSource = parseJavaScript(wheelPath);
  const styleSource = fs.readFileSync(stylePath, "utf8");

  assertE1(
    !wheelSource.includes("data:audio/wav;base64"),
    "La roue contient encore l'ancien WAV en base64."
  );
  assertE1(
    !wheelSource.includes("function ensureWheelFxStyles()"),
    "L'ancien injecteur CSS de la roue est encore présent."
  );
  assertE1(
    fs.existsSync(soundPath) && fs.statSync(soundPath).size > 44,
    "letter-wheel-spin.wav n'a pas été généré."
  );
  ...
  assertE1(
    ...
    "letter-wheel-spin.wav est invalide."
  );
  ...
  for (const marker of expectedFallbacks) {
    assertE1(appSource.includes(marker), `Noyau app.js incomplet: ${marker}`);
  }

  assertE1(
    appSource.includes("Chargement de la partie"),
    "Les anciens écrans app.js n'ont pas été remplacés par les fallbacks."
  );

  assertE1(
    bracesAreBalanced(styleSource),
    "style.css est invalide après migration."
  );

  console.log("[E1] Validation source terminée.");
}
...
function mainE1() {
  ...
  console.log("");
  console.log("[E1] Les sources propres sont maintenant écrites directement :");
  console.log(" - app.js");
  console.log(" - style.css");
  console.log(" - letter-wheel-v1.js");
  console.log(" - letter-wheel-spin.wav");
}
